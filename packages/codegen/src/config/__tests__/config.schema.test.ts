...
describe('ConfigSchema', () => {
  ...
  describe('Full Config Schema', () => {
    it('should validate a complete config with TypeScript client', () => {
      ...
      expect(firstClient?.type).toBe('typescript');
      if (firstClient?.type === 'typescript') {
        expect(firstClient.packageName).toBe('my-sdk');
      }
    });
    ...
    it('should validate config with template overrides', () => {
      ...
      expect(firstClient?.type).toBe('typescript');
      if (firstClient?.type === 'typescript') {
        expect(firstClient.templates).toBeDefined();
        expect(firstClient.templates?.['client.ts.hbs']).toBe(
