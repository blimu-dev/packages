...
describe('OpenApiService', () => {
  ...
  const mockSwaggerParser = SwaggerParser as unknown as {
    validate: ReturnType<typeof vi.fn>;
    dereference: ReturnType<typeof vi.fn>;
  ...
  describe('loadDocument', () => {
    ...
    it('should preserve internal $ref pointers when loading from URL', async () => {
      ...
      if (pathParam && !('$ref' in pathParam)) {
        const paramSchema = pathParam.schema;
        if (paramSchema && '$ref' in paramSchema) {
          expect(paramSchema.$ref).toBe('#/components/schemas/ResourceType');
        }
      }
    });
